...
import { getAccessToken } from '@/activitypub-auth';
...
export const api = ((endpoint: string, data: Record<string, any> = {}, token?: string | null | undefined) => {
	...
	const promise = new Promise((resolve, reject) => {
		void token;
		getAccessToken().then(accessToken => fetch(endpoint.indexOf('://') > -1 ? endpoint : `${apiUrl}/${endpoint}`, {
			method: 'POST',
			headers: {
			...
			const body = res.status === 204 ? null : await res.json().catch(() => ({
				error: { message: `API request failed (${res.status})`, code: 'INVALID_RESPONSE' },
			}));
...
export const apiGet = ((endpoint: string, data: Record<string, any> = {}) => {
	...
	const promise = new Promise((resolve, reject) => {
		getAccessToken().then(accessToken => fetch(`${apiUrl}/${endpoint}?${query}`, {
			method: 'GET',
			headers: {
				'Accept': 'application/json',
				...(accessToken ? { 'Authorization': `Bearer ${accessToken}` } : {}),
