/* prismjs 1.30.0 (MIT); generated from the locked Misskey 12.119.2 dependency. */
...
function l(e, t) {
	...
	n.classList.add("token", t.type);
	let r = Array.isArray(t.alias) ? t.alias : t.alias ? [t.alias] : [];
	for (let e of r) n.classList.add(e);
	let i = Array.isArray(t.content) ? t.content : [t.content];
	for (let e of i) l(n, e);
	e.append(n);
}
function u(e, t) {
	for (let t of [...e.classList]) t.startsWith("language-") && e.classList.remove(t);
	e.classList.add(`language-${t}`);
}
function d(e, t, n) {
	if (!(e instanceof HTMLElement) || typeof t != "string") throw Error("MISSKEY_CODE_HIGHLIGHT_CONFIGURATION_INVALID");
	let r = typeof n == "string" ? n : "", i = c.default.languages[r] ? r : "js", a = document.createDocumentFragment();
	for (let e of c.default.tokenize(t, c.default.languages[i])) l(a, e);
	return e.replaceChildren(a), u(e, i), e.parentElement?.tagName === "PRE" && u(e.parentElement, i), e.dataset.prismLanguage = i, i;
}
//#endregion
export { d as highlight };
